...
sys.on('loaded', function(){
	var site = new Site({
		...
		staticExts: [
			'js', 'json', 'css', 'png', 'gif', 'ico', 'jpg', 'htm', 'map', 'html', 'xml', 'txt',
	...
	site.onRequest = function(q, doc){
		...
		var home = (q.domain.indexOf('.lh')+1)?'http://io.lh/':'https://io.cx/',
				libs = (q.domain.indexOf('.lh')+1)?'http://libs.lh/':'https://libs.io.cx/';
		
		
		var patha = ((q.path || '').replace(/\/\.+/g,'').replace(/^\/|\/$/g, '') || this.index);

		var doc;

		if(!this.seo){
			q.filePath = this.path+'/'+patha;
		...
		if(q.p[0]){
		  ...
		  path = 'wiki/'+tag;
			...
			/*
			q.res.writeHead(301, {
				Location: 'https://'+q.domain+'/pictures'
			});
			q.res.end();
			*/
			site.send(q, doc);
				...
				*/
				...
				$("a[href^='/']").each((i, elem) => {
					...
					if(host == baseHost && elem.attribs.href.indexOf('wiki') == 1 && elem.attribs.href.split('/').length == 3)
						$elem.attr('href', 'https://'+q.domain+'/'+elem.attribs.href.split('/').pop());
					else
						$elem.attr('href', 'https://'+q.domain+'/'+protocol+'/'+host+elem.attribs.href);
				});
